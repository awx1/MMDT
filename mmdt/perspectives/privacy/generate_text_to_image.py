...
def generate(model_id, scenario, task, seed):
    file_path = ""
    if (task in ['LAION_1K', "REDCAPS"]):
        # Adjusted path to read data from the correct directory
        file_path = os.path.join('../../data/text_to_image/privacy', task, f'caption_names.jsonl')
    elif task in ["Sorry-Bench"]:
        # Adjusted path to read data from the correct directory
        file_path = os.path.join('../../data/text_to_image/privacy', task, f'caption_names_filtered.jsonl')
